                ...
                setTimeout(() => {
                    ...
                    const launchAllGroups = setInterval(() => {
                        ...
                        if (btnLaunch.length) {
                            ...
                            // VÝPOČET NÁHODNÉHO ZPOŽDĚNÍ (3-9 minut)
                            let randomDelayMinutes = Math.floor(Math.random() * (9 - 3 + 1) + 3);
                            let totalDelaySeconds = (120 * 60) + (randomDelayMinutes * 60);
                            
                            console.log(`Ghost Mode: All groups processed.`);
                            console.log(`Ghost Mode: Next run in 120min + random delay ${randomDelayMinutes}min.`);
                            
                            clearInterval(launchAllGroups);
                            
                            setTimeout(() => {
                                console.log("Ghost Mode: Re-triggering sequence via reload...");
                                location.reload(); 
...
if (localStorage.getItem("timeElement") == null || localStorage.getItem("timeElement") === "Date") {
    localStorage.setItem("timeElement", "Hours");
    tempElementSelection = "Hours";
} else {
...
var squad_requests = [], enabledCategories = [], time = {'off': 1.99, 'def': 1.99};
...
function readyToSend() {
    ...
    localStorage.setItem("troopTypeEnabled", JSON.stringify(troopTypeEnabled));
    localStorage.setItem("keepHome", JSON.stringify(keepHome));
    localStorage.setItem("runTimes", JSON.stringify({"off": 1.99, "def": 1.99}));
    localStorage.setItem("timeElement", "Hours");
...
let html = `<div id="massScavengeSophie" class="ui-widget-content" style="width:600px;background-color:${backgroundColor};z-index:50;position:fixed;padding:10px;border:1px solid ${borderColor};"><button class="btn" onclick="closeWindow('massScavengeSophie')">X</button><table class="vis" style="width:100%"><tr><td colspan="10" style="text-align:center; background-color:${headerColor}"><font color="${titleColor}">Mass Scavenge Ghost 0.7 (Random Delay)</font></td></tr><tr id="imgRow"></tr></table><table class="vis" style="width:100%"><tr><td colspan="4" style="text-align:center;background-color:${headerColor}"><font color="${titleColor}">Kategorie</font></td></tr><tr><td>Líný <input type="checkbox" id="category1" ${categoryEnabled[0]?'checked':''}></td><td>Běžný <input type="checkbox" id="category2" ${categoryEnabled[1]?'checked':''}></td><td>Chytrý <input type="checkbox" id="category3" ${categoryEnabled[2]?'checked':''}></td><td>Velký <input type="checkbox" id="category4" ${categoryEnabled[3]?'checked':''}></td></tr></table><table class="vis" style="width:100%"><tr><td><input type="radio" id="timeSelectorDate" name="timeSelector"> Datum</td><td><input type="date" id="offDay" value="${setDayToField(4)}"><input type="time" id="offTime" value="${setTimeToField(4)}"></td></tr><tr><td><input type="radio" id="timeSelectorHours" name="timeSelector" checked> Hodiny</td><td><input type="text" class="runTime_off" id="ghost_off" value="1.99" size="3"> Off <input type="text" class="runTime_def" id="ghost_def" value="1.99" size="3"> Def</td></tr></table><center><input type="button" class="btn btnSophie" id="sendMass" onclick="readyToSend()" value="Calculate runtimes"></center></div>`;
